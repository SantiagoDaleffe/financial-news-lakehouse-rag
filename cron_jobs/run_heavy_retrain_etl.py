...
def extract_full_history(logical_date_str: str) -> str:
    ...
    if raw_df.empty: raise ValueError(f"FATAL: Database empty for window ending {target_date}.")
    ...
    if len(processed_df) < 2000:
        raise ValueError(f"FATAL: Insufficient data density. Only {len(processed_df)} rows survived.")
...
def run_optuna_search(data_path: str) -> dict:
    ...
    backtester_args = {'train_days': 252, 'step_days': 20, 'embargo_days': 5, 'n_features': 15}
    search_space = {
        "q_high": (0.60, 0.75), "q_low": (0.25, 0.40), "buy_threshold": (0.40, 0.60),
        "sell_threshold": (0.40, 0.60), "weight_buy": (1.0, 1.5), "weight_sell": (1.5, 2.5),
        "weight_neutral": (0.8, 2.0), "n_estimators": (70, 200), "max_depth": (3, 7),
        "num_leaves": (10, 50), "learning_rate": (0.01, 0.1), "min_child_samples": (50, 300),
...
def promote_heavy_challenger(data_path: str, selected_features: list, best_params: dict) -> str:
    ...
    buy_thresh, sell_thresh = best_params.pop("buy_threshold"), best_params.pop("sell_threshold")
    w_buy, w_sell, w_neutral = best_params.pop("weight_buy"), best_params.pop("weight_sell"), best_params.pop("weight_neutral")
    
    lgbm_params = {
        "objective": "multiclass", "num_class": 3, "metric": "multi_logloss",
        "verbosity": -1, "device_type": "cpu", "random_state": 1,
        "class_weight": {-1: w_sell, 0: w_neutral, 1: w_buy}, **best_params
    }
    ...
    X_train_chall, y_train_chall = chall_train_df[selected_features], chall_train_df['target']
    X_test_chall, y_test_chall = chall_test_df[selected_features], chall_test_df['target']
    ...
    with open(config_path, "r") as f: champ_config = json.load(f)
    ...
    def get_actionable_accuracy(model, X, y_true, u_buy, u_sell):
        ...
        idx_sell, idx_buy = np.where(model.classes_ == -1)[0][0], np.where(model.classes_ == 1)[0][0]
        ...
        if not actionable_preds: return 0.0
        return np.sum(np.array(actionable_preds) == np.array(actionable_trues)) / len(actionable_preds)

    champ_acc = get_actionable_accuracy(champion_model, X_test_champ, y_test_champ, champ_config['buy_threshold'], champ_config['sell_threshold'])
    ...
    with mlflow.start_run(run_name=f"Heavy_Optuna_{datetime.now().strftime('%Y-%m-%d')}") as run:
        ...
        if chall_acc > champ_acc:
            ...
            exporter.export(
                model=final_model, target_engineer=final_target_maker,
                buy_threshold=buy_thresh, sell_threshold=sell_thresh,
                lgb_params=lgbm_params, train_days=252, step_days=20, embargo_days=5
            )
